...
def initialize_weight_matrix(board):
    ...
    weight_matrix[1][0], weight_matrix[0][1], weight_matrix[1][1], weight_matrix[0][-2], weight_matrix[1][-1], weight_matrix[-2][0], weight_matrix[1][-2], \
    weight_matrix[-1][1], weight_matrix[-2][-2], weight_matrix[-2][1] ,\
    weight_matrix[-1][-2], weight_matrix[-2][-1] = -10, -10, -10, -10, -10, -10, -10, -10,-10,-10,-10,-10
...
def select_move_minimax(board, color, limit, end_time, caching=0):
    """
...
def alphabeta_min_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    possible_moves = name_ordering_heuristic(board, get_possible_moves(board, 3 - color), 3 - color, color, ordering)
    if not possible_moves:
        # terminal status, return utility value
        res = None, compute_heuristic(board, color)
        if caching == 1:
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if limit == 0:
        return None, compute_heuristic(board, color)
    # name ordering heuristics
    possible_moves = name_ordering_heuristic(board, get_possible_moves(board, color), color, color, ordering)
    if not possible_moves:
        # terminal status, return utility value
        res = None, compute_heuristic(board, color)
        if caching == 1:
...
def select_move_alphabeta(board, color, limit, caching=0, ordering=0):
    ...
    # initialize weight matrix
    initialize_weight_matrix(board)
    move, utility = alphabeta_max_node(board, color, alpha, beta, limit, caching)
    # add delay to show the process
    # time.
    # (0.5)
...
def name_ordering_heuristic(board, possible_moves, color, ai_color, ordering):
    if ordering:
        return sorted(possible_moves, key=lambda x: compute_utility(play_move(board, color, x[0], x[1]), ai_color),
                      reverse=True)
...
def run_ai():
    ...
    """
    print("Ironman AI")  # First line is the name of this AI
